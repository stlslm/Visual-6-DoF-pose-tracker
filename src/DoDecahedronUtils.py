...
class parameters():
    def __init__(self):
        ...
        with np.load('src/PTGREY.npz') as X:
...
class txt_data():
    def __init__(self):
        ...
        self.img_pnts = [0]*13
        for i in range(1,13):
            self.edge_pts_in_img_sp[i] = np.loadtxt("src/thick_edge_coord_R3/id_{}.txt".format(i),delimiter=',',dtype=np.float32)
            self.aruco_images[i]= cv2.imread("src/aruco_images_mip_maps/res_75_{}.jpg".format(i),0)
            self.img_pnts[i] = np.loadtxt("src/thick_edge_coord_pixels/id_{}.txt".format(i),delimiter=',',dtype='int16')
